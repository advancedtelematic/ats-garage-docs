...
In ATS Garage, we generally assume that updates will be OSTree images, but in fact ATS Garage can be used to send all kinds of other updates.footnote:[For more information on using ATS Garage for other types of update, please contact us at link:mailto:hello@atsgarage.com[hello@atsgarage.com].] But for now, let's look at the default OSTree update process.

.ATS Garage device update flow
****
