...
AGL's version of meta-updater is usually a few revisions behind the latest one. ATS Garage is always tested against the latest github revision on the *morty* branch of meta-updater, so we'll check out that one.

----
cd meta-updater
git checkout github/morty
cd ..
----
