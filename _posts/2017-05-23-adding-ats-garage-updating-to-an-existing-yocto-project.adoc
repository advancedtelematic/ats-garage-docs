...
* SOTA_CLIENT - Which SOTA client to install. ATS provides two options: *rvi-sota-client* is written in Rust and usually gets new features first, and *aktualizr* is written in C++, for broader compatibility. Currently, we recommend using rvi-sota-client; aktualizr is still in pre-beta.
