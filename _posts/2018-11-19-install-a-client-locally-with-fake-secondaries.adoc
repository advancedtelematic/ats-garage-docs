= Install a client locally with fake secondaries
:page-layout: page
...
You can try out OTA Connect without having to build a whole device. This can be useful for testing the system outside of actual device updates. In this guide, you will install an aktualizr binary on your local machine, create an environment where a fake device is configured, and run aktualizr from there.
...
* A `secondary_configs` directory, containing one or more JSON files representing secondaries
...
NOTE: Because the aktualizr config uses relative paths, you *must* run aktualizr from the directory you created. You can use different directories to create different fake devices, as long as each one is kept separate.
