...
* (Optional) A `secondary_configs` directory, containing one or more JSON files representing secondary ECUs.footnote:[OTA Connect is designed for automotive use cases, where there may be many distinct microcontrollers (ECUs, in automotive parlance) inside a vehicle--sometimes over 100. Updates are downloaded by one *primary* ECU, checked for authenticity, and then distributed to *secondary* ECUs throughout the vehicle.]
